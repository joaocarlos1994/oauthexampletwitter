...
public class TwitterResource {
	...
	private final String CONSUMER_KEY = "EiOxsDIAJiIuBTMrsYXww8cFC";
	private final String CONSUMER_SECRET = "EY7AeawJpJQ8fx7NOktjdQJd5Tj6xrqX6iY1gudtrn1KZ6qq2H";

	// Url que vai receber o codigo verificador do OAuth
	private final String CALLBACK_URI = "http://localhost:8080/HelloTwitter/rest/twitter/verify";
	...
	// Cria a classe por controlar o fluxo de autorizacao
	private OAuth1AuthorizationFlow getAuthorizationFlow(final String callbackUri) {
		// Tenta obter a classe da sessao do browser
		...
		// Configura a URL de callback se necessario
		if (callbackUri != null) {
			builder.callbackUri(CALLBACK_URI);
		}

		authFlow = builder.build();

		// Armazena este objeto na sessao do browser
	...
	public Response authRedirect() {
		...
		Response redirect = Response.seeOther(UriBuilder.fromUri(authorizationUri).build()).build();
		return redirect;
	}
	...
	public String timeline(@QueryParam("token") String token, @QueryParam("tokenSecret") String tokenSecret) {
		...
		Response response = client.target("https://api.twitter.com/1.1/statuses/user_timeline.json").request().get();
	...
	public String ultimoTweet(@QueryParam("token") String token, @QueryParam("tokenSecret") String tokenSecret) {
		...
		final List<TwitterStatus> list = response.readEntity(new GenericType<List<TwitterStatus>>() {
		});
